...
const ArticleCreate = () => {
  ...
  const [content, setContent] = useState([{ subContent: []}]);
  ...
  useEffect(() => {
    if(slug.length >= 3) {
      ArticleService.getBySlug(slug).then((res) => {
        if (res?.data && res?.data?.exist) {
          console.log("Slug already exist");
          setSlugValid("alert alert-danger");
        } else {
          setSlugValid("");
        }
      })
  ...
  const schema = z.object({
    ...
    type: z.string().min(1, "Sélectionner un type"),
  });
  ...
  function handleCreate (prevState, formData) {
    ...
    const parsed = schema.safeParse({ title, h1Title, lang, titleBreadcrumb, type });
    ...
    for (let i = 0; i < arrayTags.length; i++) {
      ...
      ArticleService.create(formData).then(
          (response) => {
            setLoading(false);
            setMessage("Article " + title + " a été crée");
      ...
      <form action={formAction}>
        <div className="row m-3">
          <div className="col-auto">
            <label htmlFor="type">Article</label>
            <select className="form-select" aria-label="Article" name="type" onChange={onChangeType} value={type}>
              {types?.map(type => {
          ...
          <div className={`col-auto ${slugValid}`}>
            <label htmlFor="slug">Slug</label>
            <input
                type="text"
                className={`form-control ${slugValid}`}
                name="slug"
                value={slug}
                              ...
                              <div className={'rounded border border-2 border-info-subtle m-3 p-3'}>
                                <div className="niv-2 row">
                                  ...
                                  <div style={{cursor: 'pointer', color: '#e40c0c'}}
                                       className="blog_content_niv-2 col-1"
